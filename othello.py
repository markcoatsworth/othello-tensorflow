...
class Game(object):
    ...
    def play(self, human_player):
        ...
        while self._game_turn != GAME_OVER:
            ...
            self._board.show(human_player)
            self._board.set_player_pieces(BLACK)
            self._board.set_player_pieces(WHITE)
            
            current_player = self._game_turn
            opponent = self._game_turn^3

            print("\nBlack (" + self._player_names[BLACK] + ") pieces: " + str(self._board._player_pieces[BLACK]))
            print("White (" + self._player_names[WHITE] + ") pieces: " + str(self._board._player_pieces[WHITE]))
            print("Black (" + self._player_names[BLACK] + ") available moves: " + str(self._board._available_moves[BLACK]))
            print("White (" + self._player_names[WHITE] + ") available moves: " + str(self._board._available_moves[WHITE]) + "\n")
            ...
            if current_player == human_player:
                ...
                if move_pos in self._board._available_moves[current_player]:
                    self._board.play_move(current_player, move_pos)
                    print("Human played " + move_input + " pos=" + str(move_pos))
                    self._board.set_available_moves(opponent)
                    # Check if other player passes, or if game is over
                    if len(self._board._available_moves[opponent]) == 0:
                        self._board.set_available_moves(current_player)
                        if len(self._board._available_moves[current_player]) == 0:
            ...
            # Computer turn
                ...
                move_pos = self._board._available_moves[current_player][randint(0, len(self._board._available_moves[current_player])-1)]
                self._board.play_move(current_player, move_pos)
        ...
        self._board.set_player_pieces(BLACK)
        self._board.set_player_pieces(WHITE)
        print("Black has " + str(len(self._board._player_pieces[BLACK])) + " pieces")
        print("White has " + str(len(self._board._player_pieces[WHITE])) + " pieces\n")
        if len(self._board._player_pieces[BLACK]) > len(self._board._player_pieces[WHITE]):
            print("Black wins!\n\n")
        elif len(self._board._player_pieces[WHITE]) > len(self._board._player_pieces[BLACK]):
            print("White wins!\n\n")
        else:
